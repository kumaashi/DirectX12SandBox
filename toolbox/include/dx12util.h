...
#define CHECK_DEVICE_STATUS(name) {auto eee =  dev->GetDeviceRemovedReason(); dbg("%s, GetDeviceRemovedReason()=%08X\n", name, eee); if(eee) exit(0); };

namespace dx12util {

void
print_err_hresult(HRESULT hr)
{
	switch (hr) {
	case 0x00000000:
		err("S_OK            \n");
...
{
	D3D12CreateDevice(
		NULL, D3D_FEATURE_LEVEL_11_1, IID_PPV_ARGS(&(*dev)));
	return (0);
}
...
create_srv(ID3D12Device *dev, ID3D12Resource *res,
	D3D12_CPU_DESCRIPTOR_HANDLE hcpu_srv, int mlevel = 0)
{
...
inline int
print_pipeline_state(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p)
{
...
inline int
create_pipeline_graphics_state(ID3D12Device *dev, 
...
inline int
set_pipeline_reset_rasterizer_state(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p)
{
...
inline int
set_pipeline_fillsolid(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p, BOOL value)
{
	auto & rsstate = p->RasterizerState;

	rsstate.FillMode =
		value ? D3D12_FILL_MODE_SOLID : D3D12_FILL_MODE_WIREFRAME;
	return (0);
}

inline int
set_pipeline_depth_stencil_state(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p,
...
inline int
set_pipeline_rtv_format(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p,
...
inline int
set_pipeline_dsv_format(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p,
	DXGI_FORMAT fmt)
{
	p->DSVFormat = fmt;
	return (0);
...
inline int
set_pipeline_rt_blend_state(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p,
...
inline int
set_pipeline_independent_blend(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p,
	BOOL value)
{
...
inline int
set_pipeline_prim_type(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p,
...
inline int
set_pipeline_vs_bytecode(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p,
...
inline int
set_pipeline_ps_bytecode(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p,
	const void *bytecode, size_t size)
{
	...
	LPCSTR name, UINT num_component, UINT slot, BOOL per_instance)
{
	...
	desc.InputSlotClass = (per_instance == TRUE) ?
...
inline int
set_pipeline_input_element(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p, 
...
inline int 
init_pipeline_state(D3D12_GRAPHICS_PIPELINE_STATE_DESC *p)
{
	...
	set_pipeline_input_element(p, nullptr, 0);
	set_pipeline_fillsolid(p, TRUE);
	set_pipeline_reset_rasterizer_state(p);
	set_pipeline_depth_stencil_state(p, FALSE, FALSE);
	set_pipeline_prim_type(p, D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE);
	set_pipeline_dsv_format(p, DXGI_FORMAT_D32_FLOAT);
	set_pipeline_dsv_format(p, DXGI_FORMAT_UNKNOWN);
	set_pipeline_independent_blend(p, FALSE);
	for (int i = 0 ; i < _countof(p->BlendState.RenderTarget); i++) {
		set_pipeline_rt_blend_state(p, i, FALSE);
		set_pipeline_rtv_format(p, i, DXGI_FORMAT_R8G8B8A8_UNORM);
	}
	return (0);
}
	...
	int num_sampler)
{
	...
	for (UINT i = 0 ; i < num_srv; i++) {
		...
		set_root_param_desc_table(&roots, range, 1);
	...
	for (UINT i = 0 ; i < num_cbv; i++) {
		D3D12_DESCRIPTOR_RANGE *range = &desc_ranges[range_index];
		D3D12_ROOT_PARAMETER roots = {};
		set_desc_range(range, D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, i);
		set_root_param_desc_table(&roots, range, 1);
		root_params.push_back(roots);
		range_index++;
	}
